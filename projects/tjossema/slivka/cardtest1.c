...
//card test 1 which tests the village card
int main() {
	...
	//add cards to deck to desk more iterations
	for(i=0;i<5;i++) {
	...
	for(i = 0; i < 10; i++) {
		G.hand[player][i] = village;
		memcpy(&preG,&G,sizeof(struct gameState));
		cardEffect(village,choice1,choice2,choice3,&G,i,&bonus);
		...
		//perform 2 actions (G->numActions + 2)
		//check actions are +2
		if(!asserttrue(G.numActions == preG.numActions +2,&works)) {
			printf("Increase Action by 2 error: Has %d actions when should have %d\n",G.numActions,preG.numActions + 2);
			printf("iteration: %d\n\n",i);
		}
		//village should be in discard pile
